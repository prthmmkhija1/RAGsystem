 ...
 * Ties together: embedding → vector search → LLM answer generation.
 */
...
/**
 * Full query pipeline: embed query → search → generate answer.
 * @param {string} query - User question
 * @param {Object} opts - { topK, documentId, temperature, includeMetadata }
 * @returns {Promise<Object>} { answer, sources, query, processingTime }
 */
async function queryDocuments(query, opts = {}) {
  const topK = opts.topK || 5;
  const startTime = Date.now();

  console.log(`[RAG] Query: "${query.substring(0, 80)}..." (topK=${topK})`);
  ...
  let results;
  if (opts.documentId) {
    results = await vectorStoreService.searchByDocument(queryEmbedding, opts.documentId, topK);
  } else {
    results = await vectorStoreService.searchSimilar(queryEmbedding, topK);
  }
  ...
  // 3. Generate answer with LLM using the retrieved context
  ...
  const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);
  console.log(`[RAG]   Query answered in ${elapsed}s using ${results.length} chunks`);
...
async function compareDocuments(documentIds, topic, opts = {}) {
  ...
  console.log(`[RAG] Comparing docs [${documentIds.join(', ')}] on topic: "${topic.substring(0, 60)}"`);
  ...
  if (doc1Results.length === 0 && doc2Results.length === 0) {
    return {
      comparison: 'No relevant content found in either document for the given topic.',
      doc1Sources: [],
      doc2Sources: [],
      topic,
      processingTime: `${((Date.now() - startTime) / 1000).toFixed(2)}s`
  ...
  const comparison = await llmService.generateComparison(topic, doc1Results, doc2Results);
